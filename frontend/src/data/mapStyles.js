...
// "fittrack" reuses the Light Gray base with a CSS filter tinting it toward
...
export const MAP_STYLES = {
  fittrack: { label: 'FitTrack', url: 'https://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}', maxZoom: 16, filter: 'sepia(.25) saturate(1.1) hue-rotate(-10deg) brightness(1.08)' },
...
export const MAP_STYLE_STORAGE_KEY = 'fittrack_map_style';

export function getSavedMapStyle() {
  const saved = localStorage.getItem(MAP_STYLE_STORAGE_KEY);
  return saved && MAP_STYLES[saved] ? saved : 'fittrack';
}
