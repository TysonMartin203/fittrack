...
export default function KitchenIllustration({ selected = [], onChange }) {
  ...
  // Color helpers
  const T  = '#CC8B86'; // teal (active)
  const TH = '#CC8B86'; // teal dark
  const D  = '#FFFFFF'; // surface (inactive)
  const BG = '#F9EAE1'; // dark bg
  const R  = '#CC8B86'; // rose accent
  const MU = '#F6ECE1'; // muted surface
  const MI = '#EBD9C4'; // muted inactive
  ...
  return (
    <div>
      <svg viewBox="0 0 500 310" xmlns="http://www.w3.org/2000/svg"
        ...
        {/* Wall */}
        <rect x="0" y="0" width="500" height="195" fill="#F3E3D3"/>
        {/* Floor */}
        <rect x="0" y="195" width="500" height="115" fill={BG}/>
        {/* Counter surface */}
        <rect x="8" y="182" width="484" height="16" fill="#F6ECE1" rx="3"/>
        <rect x="8" y="182" width="484" height="4"  fill="#C9A69C" rx="2"/>
        ...
        <g onClick={() => toggle('microwave')} style={{cursor:'pointer'}}>
          ...
          <circle cx="92" cy="24" r="5" fill={isOn('microwave') ? T : '#F3E3D3'}/>
          <circle cx="92" cy="36" r="4" fill={isOn('microwave') ? R : '#F3E3D3'}/>
          <circle cx="92" cy="47" r="3" fill={isOn('microwave') ? T : '#F3E3D3'}/>
        </g>

        {/* ── STOVE / OVEN  (x=12..107, y=62..180) ── */}
        <g onClick={() => { toggle('stovetop'); toggle('oven'); }} style={{cursor:'pointer'}}>
          {/* Oven body */}
          <rect x="12" y="62" width="95" height="120" fill={isOn('oven') ? MU : '#EFDFCE'} rx="5" stroke={SH('oven')} strokeWidth="1.5"/>
          {/* Stovetop strip */}
          <rect x="12" y="62" width="95" height="26" fill={isOn('stovetop') ? D : '#EFDFCE'} rx="5"/>
          {/* Burners */}
          {[[34,75,9],[64,75,9],[88,75,7]].map(([cx,cy,r],i) => (
            <g key={i}>
              <circle cx={cx} cy={cy} r={r} fill={BG} stroke={isOn('stovetop') ? T : MU} strokeWidth="1.5"/>
              <circle cx={cx} cy={cy} r={r/2} fill={isOn('stovetop') ? T : '#F3E3D3'}/>
            </g>
          ))}
          {/* Oven window */}
          <rect x="20" y="96" width="79" height="72" fill={BG} rx="3"/>
          {isOn('oven') && <rect x="20" y="96" width="79" height="72" fill="rgba(204,139,134,.06)" rx="3"/>}
          {/* Oven handle */}
          <rect x="30" y="91" width="57" height="5" fill={isOn('oven') ? R : '#C9A69C'} rx="2.5"/>
          {/* Control knobs */}
          <rect x="12" y="174" width="95" height="8" fill={isOn('oven') ? D : MI} rx="2"/>
          {[28,47,66,85].map((x,i) => (
            <circle key={i} cx={x} cy="178" r="3" fill={isOn('oven') ? (i%2===0 ? T : R) : '#F3E3D3'}/>
          ))}
        </g>
        ...
        <g onClick={() => toggle('toasteroven')} style={{cursor:'pointer'}}>
          ...
          {isOn('toasteroven') && <rect x="140" y="15" width="90" height="52" fill="rgba(204,139,134,.05)" rx="3"/>}
          ...
          <circle cx="244" cy="28" r="6" fill={isOn('toasteroven') ? T : '#F3E3D3'}/>
          <circle cx="244" cy="42" r="4" fill={isOn('toasteroven') ? R : '#F3E3D3'}/>
          <circle cx="244" cy="54" r="4" fill={isOn('toasteroven') ? R : '#F3E3D3'}/>
          {/* Tray handle */}
          <rect x="155" y="65" width="60" height="6" fill={isOn('toasteroven') ? '#C9A69C' : MI} rx="2"/>
        </g>
        ...
        <g onClick={() => toggle('airfryer')} style={{cursor:'pointer'}}>
          ...
          <circle cx="170" cy="155" r="6" fill={isOn('airfryer') ? T : '#F3E3D3'}/>
          <rect x="150" y="164" width="40" height="5" rx="2.5" fill={isOn('airfryer') ? '#C9A69C' : MI}/>
        </g>
        ...
        <g onClick={() => toggle('instantpot')} style={{cursor:'pointer'}}>
          ...
          <rect x="221" y="104" width="72" height="10" fill={isOn('instantpot') ? '#C9A69C' : MI} rx="2"/>
          {[236,257,278].map((x,i) => (
            <circle key={i} cx={x} cy="109" r="2.5" fill={isOn('instantpot') ? (i===1?T:R) : '#F3E3D3'}/>
          ))}
        ...
        <g onClick={() => toggle('blender')} style={{cursor:'pointer'}}>
          ...
          <circle cx="332" cy="175" r="3" fill={isOn('blender') ? T : '#F3E3D3'}/>
          <circle cx="344" cy="175" r="3" fill={isOn('blender') ? R : '#F3E3D3'}/>
          ...
          {isOn('blender') && <polygon points="325,164 322,104 356,104 353,164" fill="rgba(204,139,134,.08)"/>}
        ...
        <g onClick={() => toggle('ricecooker')} style={{cursor:'pointer'}}>
          ...
          <rect x="388" y="124" width="64" height="10" fill={isOn('ricecooker') ? '#C9A69C' : MI} rx="2"/>
        ...
        <g onClick={() => toggle('slowcooker')} style={{cursor:'pointer'}}>
          ...
          <rect x="20" y="222" width="136" height="14" fill={isOn('slowcooker') ? '#C9A69C' : MI} rx="4"/>
          {[45,88,131].map((x,i) => (
            <circle key={i} cx={x} cy="229" r="3.5" fill={isOn('slowcooker') ? (i===1?T:R) : '#F3E3D3'}/>
          ))}
          {/* Handles */}
        ...
        <g onClick={() => toggle('grill')} style={{cursor:'pointer'}}>
          ...
          {/* Base/pedestal */}
          <rect x="315" y="265" width="60" height="10" rx="4" fill={isOn('grill') ? '#C9A69C' : MI}/>
          {/* Legs */}
              ...
              {selected.map(id => {
                ...
                return a ? (
                  <span key={id} onClick={() => toggle(id)} style={{
                    ...
                    background:'rgba(204,139,134,0.15)', border:'1px solid var(--teal)',
